...
#$1 配置文件路径。配置文件中都包含:1、工程路径    2、TeamID   3、archivePath    4、AdHocExportOptionsPlist.plist Path  5、adhoc export path  6、AppStoreExportOptionsPlist.plist Path    7、appsotre export path   8、scheme  ***注意顺序这8个信息的顺序***

numberOfLines=$(sed -n '$=' $1) #获取行数
if [ $numberOfLines -ne 8 ]; then
    echo "*****缺乏导出必要信息*****"
    exit
fi
...
-exportPath "${adhocExportPath}"


echo "*****正在导出Release包*****"
xcodebuild -exportArchive -archivePath  $archivePath \
-exportOptionsPlist $appStoreExportPlistPath \
-exportPath "${appStoreExportPath}"
